...
public class FirstDayOfTheWeekUdf {
    ...
    public long firstDayOfTheWeek(@UdfParameter(value = "timestamp", description="Unix timestamp in milliseconds") final long timestamp,@UdfParameter(value = "shift", description="Useful when the first day of the week is other than Monday. Use -2 for Saturday and -1 for Sunday.") final int shift,@UdfParameter(value = "truncateTime", description = "Makes Hour, Minute, Second, and Millisecond 0") final boolean truncateTime) {
    ...
    public String firstDayOfTheWeek(@UdfParameter(value = "dateTimeStr", description="A formatted date and time string. e.g.: 2022-10-27 22:19:01.000") final String dateTimeStr,@UdfParameter(value = "formatPattern", description = "Format of the input dateTimeStr (Returned format will be the same as the input format). e.g.: yyyy-MM-dd HH:mm:ss.fff") final String formatPattern,@UdfParameter(value = "shift", description="Useful when the first day of the week is other than Monday. Use -2 for Saturday and -1 for Sunday.") final int shift,@UdfParameter(value = "truncateTime", description = "Makes Hour, Minute, Second, and Millisecond 0") final boolean truncateTime) {
      
        return firstDayOfTheWeek(dateTimeStr,formatPattern,formatPattern,shift,truncateTime);
    }
    
    
    @Udf(description = "Returns a formatted string of the date(time) of the first day of the week")
    public String firstDayOfTheWeek(@UdfParameter(value = "dateTimeStr") final String dateTimeStr,@UdfParameter(value = "inFormatPattern", description = "Format of the input dateTimeStr. e.g.: yyyy-MM-dd HH:mm:ss.fff") final String inFormatPattern,@UdfParameter(value = "outFormatPattern", description = "Format of the output dateTimeStr. e.g.: yyyy-MM-dd") final String outFormatPattern,@UdfParameter(value = "shift", description="Useful when the first day of the week is other than Monday. Use -2 for Saturday and -1 for Sunday.") final int shift,@UdfParameter(value = "truncateTime", description = "Makes Hour, Minute, Second, and Millisecond 0") final boolean truncateTime) {
       
        int sh = shift * -1;
        
        DateTimeFormatter formatter = DateTimeFormat.forPattern(inFormatPattern);
